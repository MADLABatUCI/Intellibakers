...
(function($, window, document, undefined) {
	...
	var pluginName = 'rotaryswitch',
		defaults = {
			...
			startValue: -1, // the original setting when the trial started
	...
	Plugin.prototype = {
		...
		*/
		calculateSwitchDeg: function() {
			...
			var curValue = this.options.startValue;
			var curDegree = curValue * this.degPerStep;
			var nextStep = (curDegree + this.degPerStep)%360;
			var prevStep = curDegree - this.degPerStep;
			if (prevStep < 0) {
				prevStep = 360 - this.degPerStep;
			} else if (prevStep == 0) {
				prevStep = 360;
			};
			var midpoint = (curDegree+180)%360; //the point directly across from the current setting

			
			if (this.mousePosition.x !== -1) {
				this.switchDeg = (radians * (180 / Math.PI) * -1) + 180;
				if ((this.switchDeg > nextStep && this.switchDeg < midpoint) || 
					(this.switchDeg > nextStep && midpoint < nextStep) ||
					(this.switchDeg < midpoint && midpoint < nextStep)){
						this.switchDeg = nextStep;		
				} else if ((this.switchDeg >= midpoint && this.switchDeg < prevStep) ||
					(this.switchDeg >= midpoint && midpoint > prevStep) ||
					(this.switchDeg < prevStep && midpoint > prevStep)){ 
					this.switchDeg = prevStep;
					if (prevStep == 360) {
						this.switchDeg = 0;
					}
				}
